...
<div class="wrapper">
		...
		<div class="page-wrapper">
            ...
            <form class="row g-3" method="POST" action="{{route('countries.update',$country->country_id)}}">
            @csrf 
            @method('PUT')
            ...
            <option value="{{ $country->country_id }}" {{$country->country_id == $country->country_id ? 'selected="selected"' : ''}}>{{ $country->title }}</option>
            </select>
            </div> 
            <!-- <div class="col-md-6">
            <label class="form-label">Country:</label>
            ...
            <input type="text" name="url_title" class="form-control" placeholder="Enter title" required autofocus="autofocus" value="{{$country->url_title}}">
            </div>
